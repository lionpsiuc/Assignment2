...
int main(int argc, char **argv)
{
    ...
    if (myid == 0)
    {
        ...
        else if (use_nonblocking)
        {
            if (use_overlapped)
            {
                sprintf(filename, "poisson2d_sol_nonblock_overlap_nx%d_np%d", nx, nprocs);
            }
            else
            {
                sprintf(filename, "poisson2d_sol_nonblock_nx%d_np%d", nx, nprocs);
            }
        }
        else
        {
            sprintf(filename, "poisson2d_sol_sendrecv_nx%d_np%d", nx, nprocs);
        }
